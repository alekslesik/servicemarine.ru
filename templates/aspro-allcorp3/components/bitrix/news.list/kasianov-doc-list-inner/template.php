			...
			<div id="<?= $areaSectionId ?>" class="doc-list-inner__section">
				...
				<div class="doc-list-inner__list  grid-list <?= $listClasses ?>">
					...
					<? foreach ($arSection['ITEMS'] as $i => $arItem) : ?>
						...
						if (isset($arItem['PROPERTIES']['DOCUMENTS']) && $arItem['PROPERTIES']['DOCUMENTS']['VALUE']) {
							$arDocFile = $arItem['PROPERTIES']['DOCUMENTS']['VALUE'];
						} else {
						   $arDocFile = false;
                  }
						?>
                        ...
                        <div><h4 class="doc-name switcher-title"><?= $arItem['NAME'] ?></h4></div>
                        <div class="sub-doc-list-inner__item ">

                        <? if($arDocFile) : ?>
                           <? foreach ($arDocFile as $docFile) : ?>

                              <?
                                 $docFile = CAllcorp3::GetFileInfo($docFile);
                                 $docFileSize = $docFile['FILE_SIZE_FORMAT'];
                                 $docFileType = $docFile['TYPE'];
                                 $docName = htmlspecialchars($docFile['DESCRIPTION']);
                                 $bDocImage = false;
                              ...
                              <div class="dosc-flex-child">
                                 ...
                                 <? endif ?>
                                 <div class="doc-list-inner__content-wrapper <?=($docFile ? 'doc-list-inner__content--with-icon' : '')?>">
                                    <div class="doc-list-inner__top">
                                       <? if ($docFile) : ?>
                                          <? if ($bDocImage) : ?>
                                             <a href="<?= $docFile['SRC'] ?>"
                                                class="doc-list-inner__name  fancy dark_link color-theme-target switcher-title"
                                                data-caption="<?= $docName ?>">
                                                <?= $docName ?>
                                             </a>
                                          <? else : ?>
                                             <a href="<?= $docFile['SRC'] ?>" target="_blank"
